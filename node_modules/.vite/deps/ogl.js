...
var Geometry = class {
  ...
  remove() {
    if (this.vao)
      this.gl.renderer.deleteVertexArray(this.vao);
...
var Program = class {
  constructor(gl2, {
    ...
    gl2.shaderSource(vertexShader, vertex2);
...
${addLineNumbers(vertex2)}`);
    ...
    gl2.shaderSource(fragmentShader, fragment2);
...
var RenderTarget = class {
  constructor(gl2, {
    ...
    for (let i = 0; i < color; i++) {
      this.textures.push(new Texture(gl2, {
        width,
        height,
        wrapS,
        wrapT,
...
var Cylinder = class extends Geometry {
  constructor(gl2, {
    ...
    const hSegs = heightSegments;
    const num = (radialSegments + 1) * (heightSegments + 1) + 2;
    const numTris = radialSegments * (2 + heightSegments * 2);
    ...
    for (var ir = 0; ir < rSegs; ir++) {
      let irn = ir + 1;
      index[ii * 3 + 0] = bci;
      index[ii * 3 + 1] = 2 + ir * hSegsAll;
      index[ii * 3 + 2] = 2 + irn * hSegsAll;
      ii++;
      ...
      index[ii * 3 + 0] = 2 + irn * hSegsAll + hSegs;
      index[ii * 3 + 1] = 2 + ir * hSegsAll + hSegs;
      index[ii * 3 + 2] = tci;
      ii++;
    }
    Object.assign(attributes, {
...
var tempVec3a$1 = new Vec3();
var tempVec3b$1 = new Vec3();
...
var Raycast = class {
  ...
  intersectBounds(meshes) {
    ...
    meshes.forEach((mesh) => {
      ...
      if (mesh.geometry.raycast === "sphere" && mesh.geometry.bounds === Infinity)
      ...
      if (mesh.geometry.raycast === "sphere") {
        distance2 = this.intersectSphere(mesh.geometry.bounds, origin, direction);
      } else {
        distance2 = this.intersectBox(mesh.geometry.bounds, origin, direction);
      }
      if (!distance2)
        return;
      if (!mesh.hit)
        mesh.hit = { localPoint: new Vec3() };
      mesh.hit.distance = distance2;
      mesh.hit.localPoint.copy(direction).multiply(distance2).add(origin);
      hits.push(mesh);
    });
...
function Text({
  ...
  const _this = this;
  let glyphs, buffers;
  let fontHeight, baseline, scale2;
  const newline = /\n/;
  const whitespace = /\s/;
  {
    parseFont();
...
var Flowmap = class {
  constructor(gl2, {
    ...
    function createFBOs() {
      let supportLinearFiltering = gl2.renderer.extensions[`OES_texture_${gl2.renderer.isWebgl2 ? `` : `half_`}float_linear`];
      const options = {
        width: size,
        height: size,
        type: gl2.renderer.isWebgl2 ? gl2.HALF_FLOAT : gl2.renderer.extensions["OES_texture_half_float"] ? gl2.renderer.extensions["OES_texture_half_float"].HALF_FLOAT_OES : gl2.UNSIGNED_BYTE,
        format: gl2.RGBA,
        internalFormat: gl2.renderer.isWebgl2 ? gl2.RGBA16F : gl2.RGBA,
        minFilter: supportLinearFiltering ? gl2.LINEAR : gl2.NEAREST,
        depth: false
      };
...
var GPGPU = class {
  constructor(gl2, {
    ...
    this.uniform = { value: new Texture(gl2, {
      image: floatArray,
      target: gl2.TEXTURE_2D,
      type: gl2.FLOAT,
      format: gl2.RGBA,
  ...
  addPass({
    vertex: vertex2 = defaultVertex$1,
    fragment: fragment2 = defaultFragment$1,
    uniforms = {},
    textureUniform = "tMap",
    enabled = true
  } = {}) {
...
var Polyline = class {
  constructor(gl2, {
    ...
    const geometry = this.geometry = new Geometry(gl2, Object.assign(attributes, {
